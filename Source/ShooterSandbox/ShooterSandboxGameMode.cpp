...
void AShooterSandboxGameMode::BeginPlay()
{
	...
	GetWorld()->GetTimerManager().SetTimer(energyPulse, this, &AShooterSandboxGameMode::Server_GiveEnergyToPlayers, ENERGY_PULSE_INTERVAL, true);
...
bool AShooterSandboxGameMode::GetSpawnLocationAndRotation(FVector &spawnLocation, FRotator &spawnRotation, AConstructibleSurface* &surfaceToSpawnOn, bool canBeParented, FString& constructName, AShooterSandboxCharacter* spawnerPlayer)
{
	...
	if (spawnerPlayer->currentConstructionMode == EConstructionMode::Surface)
	{
		...
		if (GetWorld()->LineTraceSingleByObjectType(hit, spawnerPlayer->FollowCamera->GetComponentLocation(),
			...
			if (Cast<AConstructibleSurface>(hit.GetActor()))
			{
				if (Cast<AConstructibleSurface>(hit.GetActor())->requiresParenting && !canBeParented)
				{
					spawnerPlayer->Client_SendAlertMessage_Implementation("Can't construct " + constructName + " on " + Cast<AConstructibleSurface>(hit.GetActor())->constructName);
					return false;
				}
				spawnLocation = hit.ImpactPoint;

				float gridSize = Cast<AConstructibleSurface>(hit.GetActor())->gridSizeInUnits;
				...
				surfaceToSpawnOn = Cast<AConstructibleSurface>(hit.GetActor());
	...
	else //if construction mode == wall
	{
		...
		if (GetWorld()->LineTraceSingleByObjectType(hit, spawnerPlayer->FollowCamera->GetComponentLocation(),
			...
			if (Cast<AConstructibleWall>(hit.GetActor()))
			{
				spawnLocation = hit.ImpactPoint;

				float gridSize = Cast<AConstructibleWall>(hit.GetActor())->gridSizeInUnits;
				float padding = Cast<AConstructibleWall>(hit.GetActor())->gridBuildPadding;
